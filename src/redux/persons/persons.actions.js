 ...
 */
export const AddNewPersonsAction = personData => async (dispatch, getState) => {
  dispatch({
...
export const updatePersonsAction = (id, personData) => async (dispatch, getState) => {
  dispatch({
    type: personsTypes.UPDATE_PERSON_REQUEST,
  });

  try {
    const { user } = getState();
    const { data } = await API_INSTANCE.put(`/person/${id}`, personData, {
      headers: {
        token: user?.user?.token,
      },
    });
    ...
    dispatch({
      type: personsTypes.UPDATE_PERSON_SUCCESS,
      payload: data,
    });
    const openNotificationWithIcon = type => {
      notification[type]({
    ...
    dispatch({
      type: personsTypes.UPDATE_PERSON_FAILED,
      payload: error,
    });
  }
};

export const getPersonsList = (page, limit, queryObj) => async (dispatch, getState) => {
  dispatch({
    type: personsTypes.GET_PERSONS_LIST_REQUEST,
  });

  try {
    const { fullname, company, persno, email, identity } = queryObj;
    const { user } = getState();
    const { data } = await API_INSTANCE.get(
      `/person?navigate[page]=${page}&navigate[limit]=${limit}&filter[fullname]=${fullname}&filter[company]=${company}&filter[persno]=${persno}&filter[email]=${email}&filter[identity]=${identity}`,
      {
        headers: {
          token: user?.user?.token,
        },
...
export const personAddressAutoCompleteAction = code => async dispatch => {
  dispatch({
    type: personsTypes.PERSON_ADDRESS_AUTO_COMPLETE_REQUEST,
  });

  try {
    const { data } = await API_INSTANCE.get(
      `https://devel.ident2be.com:1443/address/findaddress?postcode=${code}`,
      {
        headers: {
          Authorization: "Basic aTJiYXBpOlJGJD4wa051VTM2aA==",
        },
    ...
    dispatch({
      type: personsTypes.PERSON_ADDRESS_AUTO_COMPLETE_SUCCESS,
      payload: data.data,
    });
  } catch (error) {
    dispatch({
      type: personsTypes.PERSON_ADDRESS_AUTO_COMPLETE_ERROR,
      payload: error,
    });
  }
};
